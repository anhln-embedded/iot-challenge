...
static bool SECURITY_STATE = true;
...
static TimerHandle_t face_handle = NULL;
esp_err_t app_face_dectection_init();
static bool face_detected_old = false;
extern "C" void app_main()
{
    ...
    esp_rmaker_node_t *node = esp_rmaker_node_init(&rainmaker_cfg, "ESP RainMaker Device", "Camera");
    if (!node)
    {
     ...
     */
    switch_device = esp_rmaker_device_create("Camera", ESP_RMAKER_DEVICE_SWITCH, NULL);
     ...
     * parameter.
     */
    esp_rmaker_device_add_param(switch_device, esp_rmaker_name_param_create(ESP_RMAKER_DEF_NAME_PARAM, "Camera"));
...
static void app_face_update(TimerHandle_t handle)
{
    bool face_detected = false;
    face_is_detection(&face_detected);
    if (face_detected_old != face_detected)
    {
        printf("face_detected: %d\n", face_detected);
        face_detected_old = face_detected;
        if (face_detected_old && SECURITY_STATE)
        {
            esp_rmaker_raise_alert("Person discovered");
        }
    }
}
...
esp_err_t app_face_dectection_init()
{
    face_handle = xTimerCreate("face_handle", 500 / portTICK_PERIOD_MS, pdTRUE, NULL, app_face_update);
    if (face_handle)
    {
        xTimerStart(face_handle, 0);
        return ESP_OK;
    }
    return ESP_FAIL;
}
